...
def create_profile_tab(parent, username):
    ...
    if doc:
        ...
        hours_var.set(doc.get("hoursLogged", 0))
        school_entry.insert(0, doc.get("school", ""))
        bio_text.insert(tk.END, doc.get("bio", ""))
        subjects_entry.insert(0, ", ".join(doc.get("subjects", [])))
        study_style_var.set(doc.get("studyStyle", "Either"))
        study_time_var.set(doc.get("studyTime", "Afternoon"))
    ...
    def save_profile():
        try:
            ...
            update = {
                ...
                "subjects": subjects,
                "studyStyle": study_style_var.get(),
                "studyTime": study_time_var.get(),
                "location": study_location_var.get(),
                "groupSize": group_size_var.get(),
            }

            users_col.update_one({"_id": username}, {"$set": update}, upsert=True)
